...
public final class ConsoleFilter extends Filter
{
	...
	public final int decide(LoggingEvent loggingEvent)
	{
		...
		if (((String) message).startsWith("[MESSAGE]") || ((String) message).startsWith("[ERROR]") || ((String) message).startsWith("[AUDIT]")
				|| ((String) message).startsWith("[ITEM]") || ((String) message).startsWith("[UNKNOWN PACKET]") ||  ((String) message).startsWith("[WARN]"))
		{
			return DENY;
		}
